// src/pages/ambulancepages/RegisterAmbulance.jsx
import React, { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { Ambulance, Lock, Phone, User } from "lucide-react";
...
const RegisterAmbulance = () => {
  const [formData, setFormData] = useState({
    ...
    driverPhone: "",
    password: "",
    vehicleNumber: "", // Corrected typo to match controller and schema
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/v1/ambulance/register", // Replace with your actual API endpoint
        {
          ...formData,
          driverlocation: {
            type: "Point",
      ...
      setSuccess(response.data.message || "Ambulance registered successfully!");
      setFormData({
        drivername: "",
        driverPhone: "",
        password: "",
          ...
          <div className="space-y-6">
            {[
              ...
              },
              {
                ...
                icon: <Ambulance size={20} className="text-blue-400" />,
                type: "text",
              },
            ].map((field, i) => (
              <motion.div
